...
class MeterDataset(Dataset):
    ...
    def __init__(self, root='data/data3', mode='train'):
        super(MeterDataset, self).__init__()
        self.dataset = []
        #with open(f'{root}/{mode}.txt','r',encoding='utf-8') as f_r:
        #    lines = f_r.readlines()
        #    for line in lines:
        #        line = line.strip()
        ...
        image_path = f'{root}/{mode}'
        # mask_path = f'{root}/annotations/{mode}'
        for image_name in os.listdir(image_path):
            #print("image_name: ", image_name)
            name = image_name.split('.')
            if name[-1] == 'jpg':
                mask_name = name[0] + '.png'
                self.dataset.append((f'{image_path}/{image_name}', f'{image_path}/{mask_name}'))
            else:
                continue
    ...
    def __getitem__(self, item):
        ...
        #print("self.dataset[item]",self.dataset[item])
        ...
        # 根据数据处理的时候对标签的输出数值来做出判断，其中mask=1是一个标签类别，mask=2是另一个标签类别
        # 把mask等于1和2的两个类别分开来放，分成两层，也就是原来放1层的数据，现在放两层
        condition = mask == 1 #mask=1 是刀闸分的标签
        mask_t[0, condition] = 1
        condition = mask == 2 #mask=2 是刀闸合的标签
