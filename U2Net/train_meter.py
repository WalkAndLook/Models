...
from utiles.dataset_meter import MeterDataset
from utiles.loss_function import DiceLoss, FocalLoss

import torch.nn as nn

os.environ['CUDA_VISIBLE_DEVICES'] = '0,1'
#print('now cuda4:', torch.cuda.current_device())
#使用指定的GPU及GPU显存
...
class Trainer(object):
    ...
    def __init__(self):
        self.net = U2NET(3, 2)
        #(3,2) 改成(3,3) 输出3个mask，同时需要改变mask的形状，在dataset.py的类函数里面
    ...
    def __call__(self):
        ...
        for epoch in range(epoch_num):
            ...
            for i, (images, masks) in enumerate(data_loader_train):
                ...
                images = images.cuda()
                masks = masks.cuda()
                #print('now cuda:', torch.cuda.current_device())
                d0, d1, d2, d3, d4, d5, d6 = self.net(images)
                # print("d0_shape:",d0.shape)  #torch.Size([10, 3, 416, 416])
                # print("masks_shape:",masks.shape)   #torch.Size([10, 3, 416, 416])
...
if __name__ == '__main__':
    ...
    trainer = Trainer()
    #print('now cuda1:', torch.cuda.current_device())
    trainer()
